...
const FilaVentas = ({
    ...
    if (send) {
      query = await swal({
        ...
        if (validateEmail(email)) {
          ...
          swal(
            'No válido!',
            'El email que colocó no es valido! Intentelo nuevamente.',
            'error',
          );
          seguir = false;
        }
      ...
      await axios
        ...
        .catch((error) => {
          ...
          if (send) {
            swal(
              'Envío de factura',
              'Hubo un error al querer envíar la factura!',
              'error',
            );
          } else {
            swal(
              'Reimpresión de factura',
              'Hubo un error al querer reimprimir la factura!',
              'error',
            );
          }
        });
  ...
  return (
    <tr
      key={id}
      style={parseInt(item.id_fact_asoc) !== 0 ? { background: '#e8e8e8' } : {}}
    >
      <td style={{ textAlign: 'center' }}>
        {moment(item.create_time).format('DD/MM/YYYY HH:mm') + ' hs'}
      </td>
      <td style={{ textAlign: 'center' }}>
        {item.raz_soc_cliente === ''
          ? 'Consumidor Final'
          : item.raz_soc_cliente}{' '}
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu className="dropdown-menu-arrow" right>
              ...
              <DropdownItem
                ...
                disabled={
                  parseFloat(item.total_fact) < 0 || parseInt(item.t_fact) < 0
                    ? true
                    : false
                }
              >
