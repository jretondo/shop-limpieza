...
const Ventas = ({ setValidPV }) => {
  ...
  const generarFactura = async () => {
    ...
    if (
      ...
      if (
        ...
        swal(
          'Error: Consumidor Final!',
          'Cuando el importe supere los $15.795,00 se tendrá que identificar el consumidor final si o si por exigencias de AFIP.',
          'error',
        );
      } else {
  ...
  const facturar = async (data) => {
    ...
    if (descuentoPerc === 0) {
      ...
      await axios
        .post(UrlNodeServer.invoicesDir.sub.codigoDescuento, data, {
          headers: {
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            swal(
              'Error en el código!',
              'Algo falló al querer generar el código.',
              'error',
            );
          }
        })
